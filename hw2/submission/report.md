...
The policy succeeds (reward $\geq 200$) in **17 of 20** episodes, with a mean reward of **262.52 $\pm$ 58.30**. The comparatively modest standard deviation (no episode falls below 81) indicates that the deterministic policy is well-behaved across unseen seeds — a direct benefit of the tanh-squashed parametrization, where the deterministic action $\tanh(\mu(s))$ is exactly the mean of the stochastic policy trained against.
