"""
Task 1: DQN (Deep Q-Network) for LunarLander-v3
================================================
Implements:
- Neural network Q-function (MLP)
- Experience replay buffer
- Target network (hard update every N steps)
- Epsilon-greedy exploration
- Training loop with reward and Q-value tracking
- Saves plot1.jpg (reward curve), plot2.jpg (Q-value curve), video.mp4
"""
...
TARGET_UPDATE  = 1000           # hard update every N steps
HIDDEN         = 128
...
def train():
    ...
    print(f"Training DQN on {ENV_NAME} for {EPISODES} episodes  [device={DEVICE}]")
    ...
    for ep in range(EPISODES):
        ...
        while not (done or truncated):
            q_val = agent.get_initial_q(state)
            step_max_qs.append(q_val)
...
def plot_rewards(rewards, filename):
    ...
    plt.title('DQN Training Reward — LunarLander-v3')
    plt.xlabel('Episode')
...
def plot_q_values(initial_q, avg_max_q, filename):
    ...
    plt.title('Q-value Curves — DQN LunarLander-v3')
    plt.xlabel('Episode')
    plt.ylabel('Q-value')
    plt.legend()
    plt.grid(True)
